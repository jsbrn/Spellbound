...
public class DungeonNSHallwayGenerator extends StoneRoomGenerator {

    private Random rng;
    private int min = 2, max = 10;

    public DungeonNSHallwayGenerator() {
        this.rng = new Random();
    }
    ...
    public byte getBase(int x, int y) {
        ...
        if (Math.abs(half-x) <= 2) return Tile.STONE_FLOOR; else return super.getBase(x, y);
    }
    ...
    public byte getTop(int x, int y) {
        ...
        if (y < getMinimum() && x == half - 2) return Tile.STONE_WALL_WEST;
        if (y < getMinimum() && x == half + 2) return Tile.STONE_WALL_EAST;
