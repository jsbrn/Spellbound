...
public class MiniMap extends GUIElement {
    ...
    private Region getRegion() {
        return region == null ? MPClient.getWorld().getRegion(Camera.getLocation()) : region;
    }
    ...
    protected void drawBuffered(Graphics b, boolean mouseHovering, boolean mouseDown) {
        b.drawImage(Assets.getImage("gui/minimap.png"), 0, 0);
    ...
    public void drawOver(Graphics g) {
        ...
        for (int x = 0; x < current.getSize(); x++) {
            for (int y = 0; y < current.getSize(); y++) {
                ...
                Image icon = Assets.getImage(current.isChunkDiscovered(x, y) ? generator.getIcon() : "gui/icons/minimap/unknown.png");
