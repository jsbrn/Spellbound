...
public class MiscMath {
    ...
    public static double tween(double start, double current, double target, double amount_to_add, double per_x_seconds) {
        double per_frame = getConstant(amount_to_add, per_x_seconds);
        if (target - start > 0) {
            return Math.min(current + per_frame, target);
        } else if (target - start < 0) {
            return Math.max(current - per_frame, target);
        }
        return current;
    }
